...
const SearchResult: React.FC<WordConentProps> = ({wordDetails}) => {
    const {meanings, phonetics} = wordDetails
            ...
            {!phonetics ? (
                ...
                (<>
                    <section className={"px-10 md:px-[200px] py-8 flex items-center justify-between"}>
                        <div>
                            <div
                                className="font-[Cooper] font-thin  text-[30px] dark:text-gray-200">{wordDetails.word}</div>
                            <h6 className="text-purple-500 py-2 text-[20px] ">/{phonetics[0]?.text}/</h6>
                        </div>
                        <div>
                            <button
                                onClick={handleClick}
                    ...
                    <section className={"container-app flex items-center md:w-full"}>
                        ...
                        <h2 className="font-[Cooper] absolute  bg-white dark:bg-black dark:text-gray-100 w-[5.5rem]  h-14   text-[#1a1a1a] text-[1.5rem] pt-2">  {meanings[0]?.partOfSpeech}</h2>
                    </section>
                    <section className={"container-app py-12"}>
                        <div className={" pt-14"}>
                            <h4 className="text-gray-500 font-[serif] text-xl font-medium pb-2">Meaning</h4>
                            <ul className="">
                                {meanings[0]?.definitions.map((item: { definition: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
                        ...
                        <div className={"flex items-center gap-24 pt-12"}>
                            <div>
                                ...
                                {meanings[0]?.synonyms.map((item: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined, index: React.Key | null | undefined) => (
                    ...
                    <section className={"container-app py-4"}>
                        <div className={" pt-14"}>
                            <h4 className="text-gray-500 font-[serif] text-xl font-medium pb-2">Meaning</h4>
                            <ul className="">
                                {meanings[1]?.definitions.map((item: { definition: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
